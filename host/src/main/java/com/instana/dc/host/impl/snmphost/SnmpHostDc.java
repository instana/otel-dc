    ...
    private void queryFileSystemUsage() throws IOException {
        ...
        for (Map<OID, SnmpValue> result1 : result) {
            ...
            if (device != null && unit != 0) {
                ...
                long free = all - used;
                output.add(new SimpleQueryResult(used).setKey(device + ":u")
                        .setAttribute("device", device).setAttribute("mountpoint", device).setAttribute("state", "read")
                        .setAttribute("mode", "-").setAttribute("type", "-")
                );
                output.add(new SimpleQueryResult(free).setKey(device + ":f")
